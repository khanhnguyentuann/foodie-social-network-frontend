...
const fetchInitialData = async () => {
    ...
    try {
        const { data: recipeData } = await axios.get(`/api/favorite/${recipeId}`, { params: { userId } });
        recipe.value = {
            ...recipeData,
...
const goToUserProfile = (userId) => {
    router.push(userId === userStore.user.id ? { name: 'MyProfile' } : { name: 'OtherProfile', params: { userId } });
};
</script>
...
<template>
    <div v-if="recipe" class="recipe-card card mt-3 p-3" style="background-color: rgba(255, 255, 255, 0.12);">
        ...
        <div class="d-flex align-items-center mb-1">
            ...
            <img :src="recipe?.user.avatar ? '/api/' + recipe.user.avatar : ''" alt="User Avatar"
                class="ml-3 click user-avatar" @click="goToUserProfile(recipe.user_id)">
